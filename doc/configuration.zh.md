...
| `CUDA_VISIBLE_DEVICES` | 空 | 可选 NVIDIA 可见卡限制。默认不限制，compose 会请求所有可用 GPU。只有需要把容器限制到某些卡时才设置；容器内 `cuda:0` 是可见集合的第 0 张，不一定等于宿主物理 GPU0。CPU-only 请设置 `DEVICE=cpu`。 |
...
Docker Compose 默认用 `count: all` 请求所有可用 NVIDIA GPU。`DEVICE=cuda`
