...
本文是 VoScript v0.8.0 的公开配置索引，覆盖当前代码已经读取并生效的
...
`MODELS_DIR` 和 `LANGUAGE` 在配置模块里有定义，但 v0.8.0 的主 HTTP 转写路径
...
这些值在 v0.8.0 还没有对应 env 或 API 字段；不要写 `WHISPER_BEAM_SIZE`、
...
v0.8.0 默认面向干净会议录音，因此 `DENOISE_MODEL=none`。只有噪声环境才建议按任务
...
- v0.8.0 的后台自动重建会保护更大的已加载或已持久化 cohort：清空转写结果、
